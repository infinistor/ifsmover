...
public class RepositoryFactory {
    public Repository getSourceRepository(String type, String jobId) {
        if (type.equalsIgnoreCase(Repository.IFS_FILE)) {
            return new IfsFile(jobId);
        } else if (type.equalsIgnoreCase(Repository.S3)) {
            return new IfsS3(jobId);
        } else if (type.equalsIgnoreCase(Repository.SWIFT)) {
            return new Swift(jobId);
        }

        return null;
    }

    public IfsS3 getTargetRepository(String jobId) {
        return new IfsS3(jobId);
    }
}
